...
export async function copyPage(d1, pg, table, after = 0) {
  ...
  if (tuples.length) await pg.begin(async (tx) => {
  ...
  return {
    table, copied: rows.length, orphaned, skipped,
    done: results.length < PAGE,
    after: byId ? last.id : after + results.length,
  };
}

// After copying, new rows must get ids above the copied ones.
...
export async function copyAll(d1, pg) {
  const report = [];
  for (const table of COPY_ORDER) {
    let after = 0, total = 0, orphanedTotal = 0, page;
    do {
    ...
    report.push({ table, copied: total, orphaned: orphanedTotal, skipped: page.skipped || [] });
  }
  await finish(pg);
  return report;
}
